...
def _check_multicommand(base_command, cmd_name, cmd, register=False):
    ...
    raise RuntimeError(
        f"{hint}. Command '{base_command.name}' is set to chain and '{cmd_name}' was added as subcommand but it in itself is a multi command. ('{cmd_name}' is a {cmd.__class__.__name__} within a chained {base_command.__class__.__name__} named '{base_command.name}')."
    )
...
class Command(BaseCommand):
    ...
    def parse_args(self, ctx, args):
        ...
        if args and not ctx.allow_extra_args and not ctx.resilient_parsing:
            ctx.fail(
                f'Got unexpected extra argument{"s" if len(args) != 1 else ""} ({" ".join(map(make_str, args))})'
            )
