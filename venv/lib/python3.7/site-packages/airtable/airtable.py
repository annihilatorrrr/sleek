...
class Airtable(object):
    ...
    def _process_response(self, response):
        try:
            ...
            if not IS_IPY and response.status_code == 422:
                err_msg = f"{err_msg.replace(response.url, unquote(response.url))} (Decoded URL)"
            ...
            try:
                ...
                if "error" in error_dict:
                    err_msg += f' [Error: {error_dict["error"]}]'
    ...
    def update_by_field(
        ...
        return self.update(record["id"], fields, typecast) if record else {}
    ...
    def replace_by_field(
        ...
        """
        record = self.match(field_name, field_value, **options)
        return self.replace(record["id"], fields, typecast) if record else {}
